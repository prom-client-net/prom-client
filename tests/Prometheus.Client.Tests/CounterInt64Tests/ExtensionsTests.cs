...
namespace Prometheus.Client.Tests.CounterInt64Tests
{
    public class ExtensionsTests
    {
        ...
        {
            var metric = Substitute.For<ICounter<long>>();
        ...
        private IMetricFamily<ICounter<long>> MockFamily()
        {
            var metric = Substitute.For<ICounter<long>>();
            var family = Substitute.For<IMetricFamily<ICounter<long>>>();
            family.Unlabelled.Returns(metric);

            return family;
        }

        private IMetricFamily<ICounter<long>, (string, string)> MockFamilyTuple()
        {
            var metric = Substitute.For<ICounter<long>>();
            var family = Substitute.For<IMetricFamily<ICounter<long>, (string, string)>>();
            family.Unlabelled.Returns(metric);
